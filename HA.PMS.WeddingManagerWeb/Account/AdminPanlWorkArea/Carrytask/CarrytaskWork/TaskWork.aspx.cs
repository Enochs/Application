...
namespace HA.PMS.WeddingManagerWeb.AdminPanlWorkArea.Carrytask.CarrytaskWork
{
    public partial class TaskWork : SystemPage
    {
        ...
        public void CostSumInsert()
        {
            ...
            pars.Add("RowType", 7, NSqlTypes.NotEquals);        //物料
            pars.Add("RowType", 8, NSqlTypes.NotEquals);        //花艺
            pars.Add("RowType", 9, NSqlTypes.NotEquals);        //其他
            ...
            foreach (var item in DataLists)
            {
                ...
                foreach (var it in Dlist)
                {
                    ...
                    if (cost == null)       //为null  说明供应商还没有添加到成本表中  属于新增
                    {
                        cost = new FL_CostSum();
                        cost.Name = item.Name;
            ...
            foreach (var item in DatasList)
            {
                ...
                switch (item.RowType)
                {
                    ...
                    case 9:         //其他
                        ...
                        ObjStatementModel.RowType = 9;
                        break;
                }
                #endregion
                var Lists = ObjProductforDispatchingBLL.GetByDispatchingID(DispatchingID, 3, "", item.Name);
                foreach (var it in Lists)
                {
                    ObjStatementModel.SupplierID = it.SupplierID;
                }
