...
namespace HA.PMS.WeddingManagerWeb.AdminPanlWorkArea.Invite.Customer
{
    public partial class InviteCommunicationContent : SystemPage
    {
        ...
        public void CreateHandle()
        {
            ...
            var OrderModel = ObjOrderBLL.GetbyCustomerID(Request["CusomerID"].ToInt32());
            if (OrderModel != null)         //说明邀约成功
            {
                HandleModel.HandleContent = "电话邀约,客户姓名:" + CustomerName + ",修改状态：" + ddlInviteSate.SelectedItem.Text + "(邀约)" + ",婚礼顾问:" + GetEmployeeName(OrderModel.EmployeeID);
            }
            else
            {
