...
<asp:Content runat="server" ContentPlaceHolderID="ContentPlaceHolder1" ID="Content1">
    ...
    <div style="overflow-x: auto;height:900px">
        <div class="widget-box">
            <div class="widget-box" style="height: auto; border: 0px; overflow: auto;">
                    ...
                    <asp:Repeater ID="repCustomer" runat="server" OnItemDataBound="repCustomer_ItemDataBound" OnItemCommand="repCustomer_ItemCommand">
                        <ItemTemplate>
                            <tr <%#IsChange(Eval("OrderNextFollowDate"),Eval("IsChecks")) %> skey='QuotedPriceQuotedID<%#Eval("QuotedID") %>'>
                                <td><a target="_blank" href="/AdminPanlWorkArea/CS/Member/CustomerDetails.aspx?Sucess=1&OnlyView=1&CustomerID=<%#Eval("CustomerID") %>"><%#ShowCstmName(Eval("Bride"),Eval("Groom")) %></a>
                                ...
                                <td>
                                    <%--<asp:Label runat="server" ID="lblNextFollowDate" Text='<%#ShowShortDate(Eval("QuotedCreateDate")) %>' ToolTip='<%#Eval("Remark") %>' />--%>
                                    <asp:TextBox runat="server" ID="txtNextFollowDate" Text='<%#ShowShortDate(Eval("QuotedCreateDate")) %>' onclick="WdatePicker();" ToolTip='<%#Eval("Remark") %>' Width="70px" />
                                </td>
                                <td>
                                ...
                                <td><a target="_blank" class="btn btn-primary btn-mini <%#GetRemoveClassByOrderID(Eval("OrderID")) %>  btnSaveSubmit<%#Eval("EmployeeID")%> btnSumbmit" href="QuotedPriceListCreateEdit.aspx?SaleEmployee=<%#Request["SaleEmployee"] %>&OrderID=<%#Eval("OrderID") %>&QuotedID=<%#Eval("QuotedID") %>&CustomerID=<%#Eval("CustomerID") %>&PartyDate=<%#Eval("PartyDate") %>">制作报价单</a>
