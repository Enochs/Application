        ...
        <div class="widget-content">
            <table>
                ...
                <tr>
                    ...
                    <td colspan="2">&nbsp;&nbsp;
                        <asp:Button ID="btnSerch" runat="server" Height="27" Text="查询" CssClass="btn btn-primary" OnClick="btnSerch_Click" />
            ...
            <table class="table table-bordered table-striped table-select">
                ...
                <tbody>
                    <asp:Repeater ID="rptTelemarketingManager" runat="server" OnItemDataBound="rptTelemarketingManager_ItemDataBound" OnItemCommand="rptTelemarketingManager_ItemCommand">
                        <ItemTemplate>
                            <tr skey='FD_SaleSourcesID<%#Eval("SSID") %>' style="height: 14px;">
                                ...
                                <td>
                                    ...
                                    <%--<%#GetChannelHref(GetSourceName(Eval("Channel"))) %>--%>
                                    <%#Eval("Channel") %>
                                </td>
                                <td><%#GetGoomByID(Eval("CustomerID")) %></td>
                                <td><%#ShowPartyDate(Eval("PartyDate")) %></td>
                                <td><span style="cursor: default" title='<%#Eval("Policy") %>'><%#ToInLine(Eval("Policy")) %></span></td>
                                ...
                                <td>
                                    <label class="NoEdit"><%#GetEmployeeName(Eval("EmployeeID")) %></label>
                                </td>
                                <td style="width: 90px; text-align: center">
                                    <asp:Button Style="display: none;" ID="btnSave" CommandArgument='<%#Eval("SSID") %>' CommandName="Save" runat="server" Text="保存" CssClass="btn btn-primary btn-mini SaveItem" />
                                    <input id="Button1" type="button" value="编辑" <%#Eval("IsFinish") != null ? (Eval("IsFinish").ToString()=="True" ? "style='display:none;'" : "") : "" %> onclick="return RowEdit(this);" class="btn btn-success btn-mini" />
                                    <asp:Button Style="display: none;" ID="btnFinish" CommandArgument='<%#Eval("SSID") %>' CommandName="Finish" runat="server" Text="确认" CssClass="btn btn-danger btn-mini SaveItem" OnClientClick="return Checkfinish();" />
                                </td>
                                <%--                     <td>
