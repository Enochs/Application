<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="ServerFileUpLoad.ascx.cs" Inherits="HA.PMS.WeddingManagerWeb.AdminPanlWorkArea.Control.ServerFileUpLoad" %>
<body bgcolor="#ffffff" style="text-align: center;">
    ...
    <script type="text/javascript" language="javascript">
        function challs_flash_update() { //Flash 初始化函数
            ...
            //a.typefile = ["Images (*.gif,*.png,*.jpg,*jpeg,*.doc,*.xls)", "*.gif;*.png;*.jpg;*.jpeg,*.doc,*.xls;", "GIF (*.gif)", "*.gif;", "PNG (*.png)", "*.png;", "JPEG (*.jpg,*.jpeg)", "*.jpg;*.jpeg;", "DOC (*.doc)", "*.doc;", "XLS (*.xls)", "*.xls;"];
            //设置可以上传文件 数组类型
        ...
        function challs_flash_onComplete(a) { //每次上传完成调用的函数，并传入一个Object类型变量，包括刚上传文件的大小，名称，上传所用时间,文件类型
            ...
            //document.getElementById('show').innerHTML += name + ' --- ' + size + '字节 ----文件类型：' + type + '--- 用时 ' + (time / 1000) + '秒<br>'
        }

        function challs_flash_onCompleteData(a) { //获取服务器反馈信息事件
            document.getElementById('show').innerHTML += a;//<font color="#ff0000">服务器端反馈信息:</font>
        }
    ...
    <div>
        <object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=10,0,0,0"
            width="680" height="500" id="update" align="middle">
            <param name="allowFullScreen" value="false" />
            <param name="allowScriptAccess" value="always" />
            <param name="movie" value="/AdminPanlWorkArea/Control/ServerfileUpload/update.swf?<%=GuiKey %>" />
            <param name="quality" value="high" />
            <param name="bgcolor" value="#ffffff" />
            <embed src="/AdminPanlWorkArea/Control/ServerfileUpload/update.swf?<%=GuiKey %>" quality="high" bgcolor="#ffffff"
                width="680" height="500" name="update" align="middle" allowscriptaccess="always"
