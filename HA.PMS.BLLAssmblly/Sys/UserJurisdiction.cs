...
namespace HA.PMS.BLLAssmblly.Sys
{
    ...
    {        /// <summary>
        ...
        /// <param name="EmpLoyeeID"></param>
        /// <returns></returns>
        public bool StarUserJurisdiction(int? EmpLoyeeID)
        {
            Sys_UserJurisdiction ObjModel = new Sys_UserJurisdiction();
            Channel ObjChannelBLL = new Channel();
            var ObjChannelList = ObjChannelBLL.GetByAll();
            foreach(var ObjChannel in ObjChannelList)
            {
                ObjModel.EmployeeID = EmpLoyeeID;
                ObjModel.ChannelID =ObjChannel.ChannelID;
                ObjModel.DataPower = 3;
                ObjModel.DepartmentID = 1;
                ObjModel.IsClose = !ObjChannel.IsPublic;
                ObjModel.DataPowerMd5Key = string.Empty;
                this.StarInsert(ObjModel);
            }
            return true;
        }
